...
class InstanceAPI(BaseHandler):
    ...
    def get(self, namespace, instance_id):
        """
        ---
        ...
        self.write(
            self.parser.serialize_instance(
                Instance.objects.get(id=instance_id, namespace=namespace), to_string=False
            )
        )

    @authenticated(permissions=[Permissions.INSTANCE_DELETE])
    def delete(self, namespace, instance_id):
        """
        ---
        ...
        Instance.objects.get(id=instance_id, namespace=namespace).delete()

        self.set_status(204)

    @coroutine
    @authenticated(permissions=[Permissions.INSTANCE_UPDATE])
    def patch(self, namespace, instance_id):
        """
        ---
        ...
        """
        response = {}
        instance = Instance.objects.get(id=instance_id, namespace=namespace)
